...
import com.javainuse.interceptor.config.LoggerInterceptor;
import com.javainuse.interceptor.config.Vani;
import com.javainuse.interceptor.config.VaniHelp;
...
public class CsiDBAppender extends DBAppender {
	...
	//final static Logger logger = LoggerFactory.getLogger(CsiDBAppender.class);
...
/*	 static final int tenant_INDEX = 1; 
	 ...
	 protected void subAppend(ILoggingEvent event, Connection connection, 
	   PreparedStatement insertStatement) throws Throwable { 
		 System.out.println("MDC value" +MDC.get("uri"));
	...
	/*	 System.out.println("URI value" +filterMapper.getUri());
	  stmt.setString(requestid_INDEX, event.getMessage()); 
	  stmt.setString(uri_INDEX, event.getMessage()); 
	...
	*/  
		...
		//VaniHelp vaniHelp = new VaniHelp();
		 Vani vani= new Vani();
		 String name = vani.getModulename();
	  System.out.println("name+++++++++++"+ name);
	  System.out.println(MDC.get("server"));
	  System.out.println(MDC.get("uri"));
	  String url = MDC.get("uri").toString();
	  String server =MDC.get("server").toString();
